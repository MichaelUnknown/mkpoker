...
TEST_CASE("game_gamecards_ctor_arr", "[tgame]")
{
    ...
    REQUIRE_THROWS_AS(static_cast<void>(gamecards<2>(board, hands2_dup)), std::runtime_error);
    REQUIRE_THROWS_AS(static_cast<void>(gamecards<6>(board, hands6_dup)), std::runtime_error);
    REQUIRE(c2a == c2a);
    REQUIRE(c2b == c2b);
    REQUIRE(c2a != c2b);
    REQUIRE(c6a == c6a);
